...
{  
 ...
 //-----------Copy Headers to first row of new sheet--------------------------
  ...
  responseSheet.getRange("A1:BR2").copyTo(archSheet.getRange("A1:BR2"));
  
  
 //------------------Row Info-----------------
  var row = 3;
  
  var lastRow = sheet.getSheetByName("Form Responses").getLastRow();
  ...
  while(row<=lastRow)
  {
    ...
    {            
      ...
      var rangeToCopy = responseSheet.getRange("A" + row + ":BR" + row);
      var rangeToPaste = archSheet.getRange("A3:BR3");
